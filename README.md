...
- 窗口到期后**恢复原版逻辑**：未交易过且无工作站的村民重置为无业
...
（`curekeep_window_end`，随存档保存）。

**窗口期内**：Mixin 给 `ResetProfession` 的决策方法（`lambda$create$0`）加了一个
新条件——若当前游戏时间未到窗口到期时间，整个行为直接跳过，职业保留。
窗口到期后标记自然失效，原版条件照常生效，重置恢复。

不再篡改任何实体数据（不做 xp 技巧），到期逻辑由条件本身完成，无需定时器。

与原版语义一致的两个例外（窗口到期后由原版条件自然保证）：
- 窗口期内**交易过**的村民（xp > 0）：原版对交易过的村民本来就不重置
- 窗口期内**认领了工作站**的村民：`JOB_SITE` 存在，`ResetProfession` 不会命中

## 配置

`run/config/curekeep-common.toml`：
...
5. 对照：不带本模组时，同一操作 1 tick 后村民就变无业（配合 `curevillager`
